  ...
  return (
    <div>
      ...
      {!surveys.length?<div className="empty"><div className="empty-icon">○</div><div>No hay encuestas disponibles aún.</div></div>:(
        <div className="card">
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>{surveys.map(s=><div key={s.id} onClick={()=>setSelectedId(s.id)} style={{padding:"14px 18px",border:`1px solid ${selectedId===s.id?"var(--accent)":"var(--border)"}`,borderRadius:8,cursor:"pointer",background:selectedId===s.id?"var(--accent-light)":"var(--bg)",transition:"all 0.15s"}}><div style={{fontWeight:500,fontSize:"0.9rem"}}>{s.title}</div><div style={{fontSize:"0.76rem",color:"var(--muted)",marginTop:4}}>{s.category} · {s.questions.length} preguntas</div></div>)}</div>
          {selectedId && !surveys.find(s=>s.id===selectedId)?.anonymous && <div className="field"><label className="label">Tu nombre o área</label><input className="input" value={respondent} onChange={e=>setRespondent(e.target.value)} placeholder="Ej: María García / Área Administración"/></div>}
          {selectedId && surveys.find(s=>s.id===selectedId)?.anonymous && <div style={{background:"var(--neutral-bg)",border:"1px solid var(--border)",borderRadius:8,padding:"12px 16px",marginBottom:16,fontSize:"0.84rem",color:"var(--neutral)"}}>🔒 Esta encuesta es anónima — no se registrará tu nombre</div>}
