...
const STYLES = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nav-logo { font-family:var(--serif); font-weight:600; font-size:1.15rem; color:var(--text); letter-spacing:0.01em; }
  .nav-logo span { color:var(--muted); font-weight:400; }
  .nav-tabs { display:flex; gap:2px; }
  .nav-tab { padding:7px 18px; border-radius:6px; border:none; cursor:pointer; font-family:var(--sans); font-size:0.82rem; font-weight:500; letter-spacing:0.02em; transition:all 0.15s; background:transparent; color:var(--muted); }
  .nav-tab:hover { background:var(--accent-light); color:var(--text); }
  .nav-tab.active { background:var(--accent); color:#fff; }
  .nav-right { display:flex; align-items:center; gap:12px; }
  .nav-badge { font-size:0.75rem; color:var(--muted); font-weight:400; border:1px solid var(--border); padding:4px 14px; border-radius:20px; background:var(--white); letter-spacing:0.02em; }
  .admin-badge { font-size:0.72rem; color:var(--positive); font-weight:600; border:1px solid #d4e8df; padding:4px 12px; border-radius:20px; background:var(--positive-bg); letter-spacing:0.04em; text-transform:uppercase; display:flex; align-items:center; gap:5px; }
  ...
  .section-sub { color:var(--muted); font-size:0.82rem; margin-top:6px; font-weight:400; }
  ...
  .input, .select, .textarea { width:100%; background:var(--white); border:1px solid var(--border); border-radius:6px; padding:10px 14px; color:var(--text); font-family:var(--sans); font-size:0.88rem; font-weight:400; outline:none; transition:border-color 0.15s; }
  ...
  .q-num { font-family:var(--serif); font-weight:600; color:var(--muted); margin-right:10px; }
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200 }}>
      <div className="login-card" style={{ position:"relative" }}>
        ...
        {error && <div className="login-error">Contraseña incorrecta. Intentá nuevamente.</div>}
        <div className="field">
          <label className="label">Contraseña</label>
          <input
            className="input" type="password" value={pwd} placeholder="••••••"
...
function AdminPanel({ surveys, loading }) {
  ...
  return (
    <div>
      <div className="section-header">
        <div><div className="section-title">Nueva encuesta</div><div className="section-sub">Completá los datos y agregá las preguntas</div></div>
            ...
            <div className="q-item" key={q.id}>
              <div className="q-item-header">
                <div style={{ flex:1, fontSize:"0.88rem", lineHeight:1.5 }}>
                  ...
                  {q.type === "multiple" && q.options && <div style={{ marginTop:6, color:"var(--muted)", fontSize:"0.78rem" }}>{q.options.join(" · ")}</div>}
                </div>
      ...
      {questions.length > 0 && title.trim() && (
        <div style={{ display:"flex", justifyContent:"flex-end" }}>
          <button className="btn btn-primary" onClick={saveSurvey} disabled={saving}>{saving ? "Guardando…" : "Guardar encuesta"}</button>
        </div>
      )}
    </div>
  );
...
function RespondSurvey({ surveys, loading }) {
  ...
  if (step === 2) return (
    <div className="success-screen">
      ...
      <div style={{ marginTop:32 }}>
        <button className="btn btn-secondary" onClick={() => { setStep(0); setSelectedId(null); setRespondent(""); setAnswers({}); }}>Responder otra encuesta</button>
      </div>
    </div>
  );
  ...
  if (step === 1 && survey) return (
    <div>
      <div className="section-header">
        <div><div className="section-title">{survey.title}</div><div className="section-sub">{survey.category} · {answered} de {total} preguntas respondidas</div></div>
      ...
      <div className="card">
        {survey.questions.map((q, i) => (
          <div className="q-response" key={q.id}>
            ...
            {q.type === "scale" && (
              <div>
                <div className="scale-row">{[1,2,3,4,5].map((v) => <button key={v} className={`scale-btn${answers[q.id]===v?" selected":""}`} onClick={() => setAnswers({...answers,[q.id]:v})}>{v}</button>)}</div>
                <div style={{ display:"flex", justifyContent:"space-between", fontSize:"0.72rem", color:"var(--muted)", marginTop:6, paddingLeft:2 }}><span>Muy bajo</span><span>Muy alto</span></div>
              </div>
            )}
  ...
  return (
    <div>
      <div className="section-header">
        <div><div className="section-title">Completar encuesta</div><div className="section-sub">Seleccioná la encuesta e ingresá tus datos</div></div>
      </div>
      {surveys.length === 0
  ...
  const exportPDF = () => {
    ...
    s.questions.forEach((q, i) => {
      ...
      if (q.type==="scale") {
        ...
        vizHtml=`<div style="display:flex;gap:16px;margin:16px 0"><div style="flex:1;background:#f0f7f4;border:1px solid #d4e8df;border-radius:8px;padding:16px;text-align:center"><div style="font-size:1.6rem;font-weight:600;font-family:Georgia,serif;color:#2d6a4f">Sí</div><div style="font-size:.9rem;color:#555">${yes} · ${Math.round(yes/tot*100)}%</div></div><div style="flex:1;background:#fdf2f2;border:1px solid #f0d4d0;border-radius:8px;padding:16px;text-align:center"><div style="font-size:1.6rem;font-weight:600;font-family:Georgia,serif;color:#c0392b">No</div><div style="font-size:.9rem;color:#555">${no} · ${Math.round(no/tot*100)}%</div></div></div>`;
...
export default function App() {
  ...
  return (
    <>
      ...
      {showLogin && <LoginModal onSuccess={handleLoginSuccess} onCancel={() => { setShowLogin(false); setPendingTab(null); }} />}
      <div className="app">
        <nav className="nav">
          <div className="nav-logo">Diagnóstico <span>/ Encuestas</span></div>
          <div className="nav-tabs">
            <button className={`nav-tab${tab==="respond"?" active":""}`} onClick={() => handleTabClick("respond")}>Responder</button>
            <button className={`nav-tab${tab==="admin"?" active":""}`} onClick={() => handleTabClick("admin")}>
              {!isAdmin ? "🔒 " : ""}Administrar
            </button>
            <button className={`nav-tab${tab==="results"?" active":""}`} onClick={() => handleTabClick("results")}>
              {!isAdmin ? "🔒 " : ""}Resultados
            </button>
          </div>
          <div className="nav-right">
            {isAdmin
              ? <><span className="admin-badge">● Admin</span><button className="logout-btn" onClick={handleLogout}>Cerrar sesión</button></>
